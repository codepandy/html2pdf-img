...
export default class HTML2JsPdf {
  ...
  outPdf(
    ...
    if (
      ...
      console.error("请传入要参与分页计算的元素=>pageBreak");
      return;
    }
    if (!this.container) {
      errorCallback &&
    ...
    if (this.option.pageBreak && this.option.pageBreak.length > 0) {
      ...
      for (let i = 0; i < allElement.length; i++) {
        let multiple = Math.ceil(
          this.getTopAndHeightTotal(allElement[i]) / pageHeight
        );
        if (this.isSplit(allElement, i, multiple * pageHeight, pageHeight)) {
          ...
          let newNode = this.createElement("div", {
            ...
            style: {
              height:
                multiple * pageHeight -
                this.getTopAndHeightTotal(el) +
                this.option.pageStartOffset +
                "px",
              width: "100%",
  ...
  isSplit(nodes, index, pageHeight, onePageHeight) {
    let clientRectNext = nodes[index + 1]
      ? nodes[index + 1].getBoundingClientRect()
      : 0;
    let nextNPages =
      Math.abs(clientRectNext.bottom - clientRectNext.top) / onePageHeight;
    // 计算当前这块dom是否跨越了a4大小，以此分割
    return (
      this.getTopAndHeightTotal(nodes[index]) < pageHeight &&
      nodes[index + 1] &&
      this.getTopAndHeightTotal(nodes[index + 1]) > pageHeight &&
      nextNPages <= 1
    );
  }
